...
func HandleRequests(){ 
	...
	// Обработка HTML-файлов (страниц) и обработка ошибок если че то пойдет не так
	...
	e.GET("/api/history", func(c echo.Context) error{
		websocket.GetHistory(c) // Возврат истории в чат 
		return nil
	})
	...
	go websocket.HandleMessages() // Горутинка вебсокета 
